describe('the blog', () => {
  ...
  it('should allow selecting a post', function() {
    ...
    cy.get('main').should('contain', 'TITLE: select title 001')
    cy.get('main').should('contain', 'BODY: select body 001')

    cy.contains('select title 002').click()
    cy.get('main').should('contain', 'TITLE: select title 002')
    cy.get('main').should('contain', 'BODY: select body 002')
